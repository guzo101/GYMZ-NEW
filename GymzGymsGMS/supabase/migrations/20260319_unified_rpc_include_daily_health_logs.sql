-- ============================================================================
-- Include daily_health_logs (steps, sleep, active minutes) in get_unified_app_data
-- Date: 2026-03-19
-- Fixes: dashboard / RPC had no SSOT for steps; app relied on stale closure values.
-- ============================================================================
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'health', COALESCE(
            (
                SELECT jsonb_build_object(
                    'steps', COALESCE(dhl.steps, 0),
                    'sleep_minutes', COALESCE(dhl.sleep_minutes, 450),
                    'active_minutes', COALESCE(dhl.active_minutes, 0)
                )
                FROM public.daily_health_logs dhl
                WHERE dhl.user_id = p_user_id AND dhl.date = p_date
                LIMIT 1
            ),
            jsonb_build_object(
                'steps', 0,
        ...
        'nutrition', (
            SELECT jsonb_build_object(
                ...
                'logs', COALESCE(jsonb_agg(l) FILTER (WHERE l.log_id IS NOT NULL), '[]'::jsonb)
            )
