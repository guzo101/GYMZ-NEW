-- ============================================================================
-- Allow event_access users to pay for gym_access plans (upgrade path)
-- Event members upgrading to full gym access must be able to purchase gym_access plans
-- ============================================================================
...
BEGIN
    ...
    -- Plan scope check: allow plan if it matches user's access mode OR if user is event_access
    -- upgrading to gym_access (plan scope = gym_access). One-way upgrade path.
    IF v_user_access_mode IS NOT NULL THEN
        IF COALESCE(v_plan.access_mode_scope, 'gym_access') = 'both' THEN
            NULL; -- both: always allowed
        ELSIF COALESCE(v_plan.access_mode_scope, 'gym_access') = v_user_access_mode THEN
            NULL; -- same path: allowed
        ELSIF v_user_access_mode = 'event_access' AND COALESCE(v_plan.access_mode_scope, 'gym_access') = 'gym_access' THEN
            NULL; -- upgrade: event -> gym allowed
        ELSE
