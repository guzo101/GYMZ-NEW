-- ============================================================================
-- GYMZ: RESTORE ULTIMATE FAILSAFE SIGNUP & PROFILE SYNC
-- Date: 2026-03-05
-- Purpose: 
-- 1. RESTORE FAILS SAFE: Re-introduce BEGIN/EXCEPTION block to ensure Auth never blocks.
-- 2. DYNAMIC SYNC: Capture names and marketing consent from metadata.
-- 3. NEUTRAL SUPPORT: allow NULL gym_id for deferred assignment.
-- ============================================================================

BEGIN;

-- 1. RESTORE handle_new_user (The Universal Gate)
...
BEGIN
    ...
    BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            v_gym_id := NULL; -- Silence cast errors (e.g. if gym_id is 'undefined' or malformed)
        END;

        -- Fallback to OAC invitations if gym_id is missing from metadata
        ...
        -- Verify gym exists to prevent FK violation
