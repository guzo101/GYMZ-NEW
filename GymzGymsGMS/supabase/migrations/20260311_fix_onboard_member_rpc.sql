-- ============================================================================
-- FIX: onboard_member_manually RPC - payments column mismatch
-- Problem: The original RPC tried to insert `currency` and `plan_name` columns
--          into the `payments` table, but those columns do not exist there.
--          `currency` lives on `gym_membership_plans`, not `payments`.
--          `plan_name` should go into the `description` field (which is how
--          the enforce_onboarding_plan_payment trigger handles it).
-- Fix:     Remove `currency` and `plan_name` from the payments INSERT.
--          Use `description` for the plan name instead.
-- ============================================================================
...
BEGIN
    ...
    -- NOTE: `currency` and `plan_name` are NOT columns on the payments table.
    -- We use `description` to store the plan name, and `amount` for the price.
    INSERT INTO public.payments (
        gym_id, user_id, amount, status, payment_method, 
        plan_id, description, created_at, updated_at
    ) VALUES (
        p_gym_id, v_user_id, v_plan_price, 'approved', 'cash', 
        p_plan_id, 'Historical payment migration - ' || v_plan_name, p_paid_at, p_paid_at
    )
