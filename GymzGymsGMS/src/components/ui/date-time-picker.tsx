            ...
            <Popover>
                <PopoverTrigger asChild>
                    <Button
                        variant={"secondary"}
                        className={cn(
                            "w-full justify-start text-left font-bold h-12 bg-secondary hover:bg-secondary/90 border-none rounded-xl px-4 min-w-0 overflow-hidden",
                            !date && "text-secondary-foreground font-bold"
                        )}
                    >

                        <CalendarIcon className="mr-2 h-4 w-4 text-secondary-foreground" />
                        {date ? (
                            <span className="text-secondary-foreground truncate">
                                {format(date, "PPP")} at {time}
                            </span>
                        ) : (
                            <span className="text-secondary-foreground truncate">{placeholder}</span>
                        )}

                    </Button>

                </PopoverTrigger>
                <PopoverContent className="w-auto p-0 bg-[#0A0F0A] border-sidebar-border/40 dark text-white" align="start">
