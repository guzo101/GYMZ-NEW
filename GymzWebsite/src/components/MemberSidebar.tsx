...
const memberLinks = [
  ...
  { to: "/member/calendar", label: "Schedule", icon: Calendar },
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (data) {
          const mappedData = DataMapper.fromDb(data);
          setMembershipStatus(mappedData.membershipStatus);
        }
      } catch (err) {
  ...
  return (
    <aside className="min-h-screen w-20 md:w-56 bg-card border-r px-2 py-4 flex flex-col gap-8">
      <div className="hidden md:flex flex-col items-center gap-2 pb-4 border-b">
        ...
        <div className="font-bold text-lg tracking-tight text-center">Member Portal</div>
      </div>
